    ...
    <section v-else class="rounded-[1.5rem] bg-white p-5 shadow-[0_10px_24px_rgba(15,23,42,0.04)] sm:p-6">
      <div class="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 class="mt-2 text-2xl font-black tracking-tight text-slate-900 sm:text-3xl">Retirer vers un numero</h1>
          <p class="mt-3 text-sm leading-6 text-slate-500">
            Saisis le montant, le numero qui sera credite, puis choisis Orange Money ou MTN MoMo.
          </p>
        </div>
      ...
      <form class="mt-5 space-y-4" @submit.prevent="submitWithdraw">
        <div class="grid gap-3 sm:grid-cols-2">
          <label class="space-y-2 sm:col-span-2">
            ...
            <p class="text-xs text-slate-400">Numero du profil: {{ profilePhone || 'non disponible' }}</p>
          </label>
...
const canWithdraw = computed(() => role.value !== 'provider')
...
watch(
  () => form.payment_method,
  (method) => {
    form.description = method === 'mtn_money' ? 'Retrait via MTN MoMo' : 'Retrait via Orange Money'
  },
  { immediate: true }
)
...
async function submitWithdraw() {
  ...
  if (!canWithdraw.value) {
    error.value = 'Les prestataires ne peuvent pas effectuer de retrait depuis cette interface.'
    return
  }
  ...
  if (!localPhone.value.trim()) {
    error.value = 'Veuillez saisir le numero a crediter.'
    return
  }
  ...
  try {
    ...
    const { data } = await api.post('/wallet/withdraw', {
      ...
      reference,
      description: form.description || 'Retrait via Orange Money',
    ...
    app.pushToast({
      ...
      message: 'Le retrait a ete enregistre.',
    })
    router.replace('/client/wallet')
  } catch (err) {
    const message = err?.response?.data?.message || 'Retrait impossible.'
    error.value = message
    app.pushToast({
      type: 'error',
