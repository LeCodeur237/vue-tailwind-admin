  ...
  <div class="space-y-6 pb-5 sm:pb-4 lg:pb-4">
    ...
    <section v-else class="rounded-[1.5rem] bg-white p-5 shadow-[0_10px_24px_rgba(15,23,42,0.04)] sm:p-6">
      <div class="flex items-start justify-between gap-4">
        <div>
          <p class="section-title">Recharge</p>
          <h1 class="mt-2 text-2xl font-black tracking-tight text-slate-900 sm:text-3xl">Recharger le wallet</h1>
          <p class="mt-3 text-sm leading-6 text-slate-500">
            Saisis le montant, le numero qui sera debite, puis choisis Orange Money ou MTN MoMo.
          </p>
        </div>
      ...
      <div class="mt-5 rounded-[1.25rem] border border-slate-200 bg-slate-50 p-4">
        <p class="text-sm text-slate-500">
          Solde actuel:
          <span class="font-semibold text-slate-900">{{ loadingBalance ? '...' : formatMoney(balance, currency) }}</span>
        </p>
      </div>
      ...
      <form class="mt-5 space-y-4" @submit.prevent="submitTopup">
        <div class="grid gap-3 sm:grid-cols-2">
          <label class="space-y-2 sm:col-span-2">
            <span class="text-sm font-semibold text-slate-700">Numéro à débiter</span>
            <div class="flex items-stretch gap-2">
              <div ref="countryMenuRef" class="relative w-[112px] shrink-0">
                ...
                >
                  ...
                  >
                    <span class="truncate text-sm font-semibold">{{ code.label }}</span>
                    <span
            ...
            <p class="text-xs text-slate-400">
              Numéro du compte: {{ profilePhone || 'non disponible' }}
            </p>
          </label>
        ...
        <button
          ...
          :disabled="loading"
        >
          {{ loading ? 'Recharge...' : 'Confirmer la recharge' }}
        </button>
      </form>
    </section>
  </div>
...
import { ArrowLeftIcon, ChevronDownIcon, WalletIcon } from '@heroicons/vue/24/solid'
...
const selectedCountryLabel = computed(() => {
  return selectedCountryCode.value || 'Pays'
})
...
watch(
  () => form.payment_method,
  (method) => {
    form.description = method === 'mtn_money' ? 'Recharge via MTN MoMo' : 'Recharge via Orange Money'
  },
  { immediate: true }
)
...
async function submitTopup() {
  ...
  if (!canRecharge.value) {
    error.value = 'Les prestataires ne peuvent pas recharger depuis cette interface.'
    return
  }

  if (!userId) {
    error.value = 'Utilisateur connecte introuvable.'
    return
  }

  const phoneNumber = composePhoneNumber(selectedCountryCode.value, localPhone.value)

  if (!localPhone.value.trim()) {
    error.value = 'Veuillez saisir le numero a debiter.'
    return
  }

  if (!isValidInternationalPhoneNumber(selectedCountryCode.value, localPhone.value)) {
    error.value = 'Veuillez saisir un numero valide selon le pays selectionne.'
    return
  }
  ...
  loading.value = true
  try {
    const reference = `TOPUP-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${Date.now()
      .toString()
      .slice(-3)}`
    ...
    const { data } = await api.post('/wallet/topup', {
      ...
      reference,
      description: form.description || 'Recharge via Orange Money',
      payment_method: form.payment_method,
    })

    if (data?.success && data?.data) {
      balance.value = Number(data.data.balance || balance.value)
      currency.value = data.data.currency || currency.value
    } else {
      await loadBalance()
    }
    ...
    app.pushToast({
      type: 'success',
      title: 'Recharge effectuee',
      message: 'Votre wallet a ete credite.',
    })
    router.replace('/client/wallet')
  } catch (err) {
    const message = err?.response?.data?.message || 'Recharge impossible.'
    error.value = message
    app.pushToast({
      type: 'error',
      title: 'Recharge refusee',
      message,
    })
  } finally {
    loading.value = false
...
watch(profilePhone, (value) => {
  if (!localPhone.value && value) {
