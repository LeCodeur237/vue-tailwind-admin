<template>
  <div class="min-w-0 space-y-4 overflow-x-hidden px-3 pb-2 sm:px-0 sm:pb-5 lg:pb-4">
    <section class="rounded-[1.5rem] bg-white p-4 sm:p-6">
      <div class="flex items-center gap-3">
        ...
        <div class="flex shrink-0 items-center gap-2">
          <span class="hidden rounded-full bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-500 ring-1 ring-slate-100 sm:inline-flex">
            {{ total }} mission(s)
          </span>
    ...
    <section v-else class="space-y-3">
      ...
      <div
        v-if="!missions.length && !loading"
        class="rounded-[1.5rem] border border-dashed border-slate-200 bg-white p-6 text-center text-sm text-slate-500 shadow-[0_10px_24px_rgba(15,23,42,0.04)]"
      >
      ...
      <p v-else-if="!hasMore && missions.length" class="py-2 text-center text-xs font-semibold uppercase tracking-[0.22em] text-slate-400">
